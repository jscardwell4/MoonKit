...
public protocol InlinePickerDelegate: class
{
  ...
  func inlinePicker(_ picker: InlinePickerView, contentForItem item: Int)
  -> InlinePickerView.Item
...
extension InlinePickerDelegate
{
  ...
  public func inlinePicker(_ picker: InlinePickerView,
...
open class InlinePickerView: UIControl
{
  ...
  override open class var layerClass: AnyClass { CATransformLayer.self }

  /// The delegate responsible for the collection's content, appearance,
  ...
  private func setup()
  {
    ...
    collectionView.register(
      InlinePickerViewLabelCell.self,
      forCellWithReuseIdentifier: "label"
    )
    collectionView.register(
      InlinePickerViewImageCell.self,
      forCellWithReuseIdentifier: "image"
    )
    collectionView.register(
      InlinePickerViewDebugCell.self,
      forCellWithReuseIdentifier: "debug"
    )
  ...
  /// Returns the width as would be returned by `UIPickerView` and height
  /// equal to `itemHeight`.
  override open var intrinsicContentSize: CGSize
  {
    return CGSize(width: 320, height: itemHeight)
  }
  ...
  /// Accessors for `font.fontName`. Setter updates `font` with using the
  /// current `pointSize`.
  @IBInspectable public var fontName: String
  {
    ...
    set
    {
      if let font = UIFont(name: newValue, size: self.font.pointSize) { self.font = font }
    }
  }

  /// Accessors for `font.pointSize`. Setter updates `font` with using the
  /// current `fontName`.
  ...
  @IBInspectable public var selectedFontName: String
  {
    ...
    set
    {
      if let font = UIFont(name: newValue, size: selectedFont.pointSize)
      {
        selectedFont = font
      }
    }
  ...
  /// Since the view is backed by an transform layer, these accessors are rerouted to
  /// the collection view.
  ...
  @IBInspectable public var marker: UIImage?
  {
    ...
    set
    {
      guard let marker = newValue?.withRenderingMode(.alwaysTemplate)
      else
      {
  ...
  /// Refreshes the item cache if the number of items reported by `delegate` doesn't
  /// match the cached count.
  private func checkItemCache()
  {
    guard let delegate = delegate,
          delegate.numberOfItems(in: self) != itemCache.expectedCount
    else { return }
    cacheItems()
  }

  /// Queries the `delegate` for the collection's items to keep in `itemCache`. Existing
  /// cache is cleared regardless of whether `delegate` continues to be valid.
  private func cacheItems()
  {
    ...
    for index in 0 ..< count
    {
      ...
      switch item
      {
        case let .text(text):
          itemCache.widths
            .append(max(text.size(withAttributes: fontAttributes).width.rounded(.up),
                        text.size(withAttributes: selectedFontAttributes)
                          .width.rounded(.up)))

        case let .image(image):
          itemCache.widths.append(image.size.height > itemHeight
                                    ? ((image.size.width / image.size.height) * itemHeight)
                                    .rounded(.up)
  ...
  @IBInspectable public var selection: Int = 0
  {
    didSet
    {
      ...
      // Handle possible change in the type of item selected when items have
      // already been cached.
      guard max(oldValue, selection, itemCache.expectedCount) == itemCache.expectedCount
      else { return }

      switch (itemCache.items[oldValue], itemCache.items[selection])
      {
        case (.text, .image),
             (.image, .text): updateBaselineView()
        case (.text, .text),
             (.image, .image): break
      }
    }
  ...
  /// Returns the frame of the cell for the currently selected item with the origin
  /// adjusted for the content offset of the collection view.
  public var selectedItemFrame: CGRect?
  {
    guard selection > -1,
          let cell = collectionView
            .cellForItem(at: IndexPath(item: selection, section: 0)),
          cell.isSelected
    else
    {
      return nil
  ...
  private lazy var baselineBottomConstraint: NSLayoutConstraint = {
    guard let result = ($0 == $1 --> Identifier(for: self, tags: "Baseline")).constraint
    else
    {
  ...
  /// Returns the value of this property for the selected item's cell or of the
  /// collection view if the cell is unavailable.
  override open var forLastBaselineLayout: UIView { return baselineView }
}
...
extension InlinePickerView: UICollectionViewDelegate, UICollectionViewDataSource
{
  ...
  {
    guard section == 0
    else { return section == 1 && InlinePickerView.enableDebugCell ? 1 : 0 }
    return itemCount
  }
                             ...
                             cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
  {
    ...
    {
      guard indexPath.section == 1, InlinePickerView.enableDebugCell
      else
      {
        fatalError("Internal inconsistency: unexpected section - \(indexPath.section)")
      }
      return collectionView.dequeueReusableCell(
        withReuseIdentifier: "debug",
        for: indexPath
      )
    }
    ...
    {
      case let .image(image):
        let imageCell = collectionView
          .dequeueReusableCell(withReuseIdentifier: "image",
      ...
      case let .text(label):
        let labelCell = collectionView
          .dequeueReusableCell(withReuseIdentifier: "label",
...
extension InlinePickerView: UIScrollViewDelegate
{
  ...
  {
    guard let activeSelection = layout.activeSelection
    else
    {
