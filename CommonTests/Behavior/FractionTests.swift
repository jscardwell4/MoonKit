...
final class FractionTests: XCTestCase {
  ...
  func testParts() {
    expect((4╱5).integerPart) == 0 //    expect(parts.nonrepeating) == [8] //    expect(parts.repeating).to(beEmpty())
    expect((4╱5).fractionalPart) == 4╱5
    expect((3╱4).integerPart) == 0 //    expect(parts.nonrepeating) == [7, 5] //    expect(parts.repeating).to(beEmpty())
    expect((3╱4).fractionalPart) == 3╱4
    expect((0╱64).integerPart) == 0 //    expect(parts.nonrepeating).to(beEmpty()) //    expect(parts.repeating).to(beEmpty())
    expect((0╱64).fractionalPart) == 0╱64
    expect((1╱4).integerPart) == 0 //    expect(parts.nonrepeating) == [2, 5] //    expect(parts.repeating).to(beEmpty())
    expect((1╱4).fractionalPart) == 1╱4
    expect((3╱8).integerPart) == 0 //    expect(parts.nonrepeating) == [3, 7, 5] //    expect(parts.repeating).to(beEmpty())
    expect((3╱8).fractionalPart) == 3╱8
    expect((9╱11).integerPart) == 0 //    expect(parts.nonrepeating).to(beEmpty()) //    expect(parts.repeating) == [8, 1]
    expect((9╱11).fractionalPart) == 9╱11
    expect((1╱24).integerPart) == 0 //    expect(parts.nonrepeating) == [0, 4, 1] //    expect(parts.repeating) == [6]
    expect((1╱24).fractionalPart) == 1╱24
    expect((8╱3).integerPart) == 2 //    expect(parts.nonrepeating).to(beEmpty()) //    expect(parts.repeating) == [6]
    expect((8╱3).fractionalPart) == 2╱3
    expect((46╱11).integerPart) == 4 //    expect(parts.nonrepeating).to(beEmpty()) //    expect(parts.repeating) == [1, 8]
    expect((46╱11).fractionalPart) == 2╱11
    expect((1002331╱24).integerPart) == 41763 //    expect(parts.nonrepeating) == [7, 9, 1] //    expect(parts.repeating) == [6]
    expect((1002331╱24).fractionalPart) == 19╱24
    expect((22╱5).integerPart) == 4 //    expect(parts.nonrepeating) == [4] //    expect(parts.repeating).to(beEmpty())
    expect((22╱5).fractionalPart) == 2╱5
    expect((98╱4).integerPart) == 24 //    expect(parts.nonrepeating) == [5] //    expect(parts.repeating).to(beEmpty())
    expect((98╱4).fractionalPart) == 2╱4
    expect((63╱8).integerPart) == 7 //    expect(parts.nonrepeating) == [8, 7, 5] //    expect(parts.repeating).to(beEmpty())
  ...
  func testInitializeWithSignExponentSignificand() {
    ...
    expect(Fraction(sign: .minus, exponent: -38, significand: UInt128(low: 0xf0884a91745d1745, high: 0x3d8d9bcf8fe2a0c0)╱1)) == -(9╱11)
    expect(Fraction(sign: .minus, exponent: -38, significand: UInt128(low: 0x15bb16c2aaaaaaaa, high: 0x322788703c59d85)╱1)) == -(1╱24)
    expect(Fraction(sign: .minus, exponent: -37, significand: UInt128(low: 0x57e091aaaaaaaaaa, high: 0x140fd02ce4f0bced)╱1)) == -(8╱3)
    expect(Fraction(sign: .minus, exponent: -37, significand: UInt128(low: 0xecb77011745d1745, high: 0x1f75e38c3879855c)╱1)) == -(46╱11)
    expect(Fraction(sign: .minus, exponent: -33, significand: UInt128(low: 0xc1aa8dc5eaaaaaaa, high: 0x1f6b69e7fe7a5dad)╱1)) == -(1002331╱24)
  ...
  func testInitializeWithSignMagnitude() {
    let mSign = Fraction(numerator: 1, denominator: 1, sign: .minus)
    let pSign = Fraction(numerator: 1, denominator: 1, sign: .plus)

    expect(Fraction(signOf: pSign, magnitudeOf: 1╱3)) == 1╱3
    expect(Fraction(signOf: mSign, magnitudeOf: 1╱3)) == (-1╱3)
    expect(Fraction(signOf: pSign, magnitudeOf: (-1╱3))) == 1╱3
    expect(Fraction(signOf: mSign, magnitudeOf: (-1╱3))) == (-1╱3)
    expect(Fraction(signOf: pSign, magnitudeOf: Fraction.infinity)) == Fraction.infinity
    expect(Fraction(signOf: mSign, magnitudeOf: Fraction.infinity)) == -Fraction.infinity
    expect(Fraction(signOf: pSign, magnitudeOf: (-Fraction.infinity))) == Fraction.infinity
    expect(Fraction(signOf: mSign, magnitudeOf: (-Fraction.infinity))) == -Fraction.infinity
    expect(Fraction(signOf: pSign, magnitudeOf: Fraction.zero)) == Fraction.zero
    expect(Fraction(signOf: mSign, magnitudeOf: Fraction.zero)) == -Fraction.zero
    expect(Fraction(signOf: pSign, magnitudeOf: (-Fraction.zero))) == Fraction.zero
    expect(Fraction(signOf: mSign, magnitudeOf: (-Fraction.zero))) == -Fraction.zero
    expect(Fraction(signOf: pSign, magnitudeOf: Fraction.nan)).to(beNaN())
    expect(Fraction(signOf: mSign, magnitudeOf: Fraction.nan)).to(beNaN())
    expect(Fraction(signOf: pSign, magnitudeOf: Fraction.signalingNaN)).to(beNaN())
    expect(Fraction(signOf: mSign, magnitudeOf: Fraction.signalingNaN)).to(beNaN())
  }
  ...
  func testEqualities() {
    ...
    let a = -Fraction.nan
    let b = -Fraction.signalingNaN
    let c = -Fraction.infinity
    let d = -2╱3
    let e = -1╱3
    let f = -Fraction.zero
    let g = Fraction.zero
